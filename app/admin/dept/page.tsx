...
import {
  IRRIGATION_CSV_HEADER,
  IRRIGATION_ORG_STATS_FIELDS,
  irrigationOrgStatValue,
} from '../../../lib/irrigationOrgStats';
              ...
              <section className="rounded-lg border border-border bg-background p-4">
                ...
                >
                  ...
                  <div className="rounded-md border border-border/70 bg-slate-50/80 p-3">
                    <p className="text-xs font-semibold text-text">Organization summary stats</p>
                    <p className="mt-1 text-[11px] text-text-muted">
                      Shown in the public portfolio highlights (Total Panchayat Covered, Total Aycut, Total
                      Beneficiaries, Total Crops).
                    </p>
                    <div className="mt-3 grid gap-3 md:grid-cols-2">
                      {IRRIGATION_ORG_STATS_FIELDS.map((field) => (
                        <div key={field.key} className="space-y-1">
                          <label className="block text-text">{field.header}</label>
                          <input
                            className="w-full rounded-md border border-border bg-background px-2 py-1.5 text-xs outline-none focus:border-primary"
                            value={irrigationFormValues[field.key] ?? ''}
                            onChange={(e) =>
                              setIrrigationFormValues((prev) => ({
                                ...prev,
                              ...
                              {deptCode === 'IRRIGATION' && (
                                <button
                                  ...
                                  onClick={async () => {
                                    ...
                                    IRRIGATION_ORG_STATS_FIELDS.forEach((field) => {
                                      const k = snakeFromHeader(field.header);
                                      const statVal = existingProfile
                                        ? irrigationOrgStatValue(existingProfile, field)
                                        : null;
                                      if (statVal != null) vals[k] = v(statVal);
                                    });
                                    const nameKey = snakeFromHeader('WORK NAME');
                                    const latKey = snakeFromHeader('LATITUDE');
                                    const lngKey = snakeFromHeader('LONGITUDE');
