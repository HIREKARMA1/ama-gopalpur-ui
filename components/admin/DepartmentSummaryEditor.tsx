  ...
  return (
    <section className="rounded-lg border border-border bg-background p-4 text-sm">
      ...
      <p className="mt-1 text-xs text-muted-foreground">
        Manage department name (Odia) and overview text. Minister cards are managed in the Ministers section above.
      </p>
      <form onSubmit={submit} className="mt-3 grid gap-3">
        ...
        <input
          ...
          placeholder="Department name (Odia)"
          value={form.department_name_od}
          onChange={(e) => setForm((p) => ({ ...p, department_name_od: e.target.value }))}
        />
        <textarea
          className="rounded border border-border px-3 py-2 text-xs"
          rows={4}
          placeholder="Overview paragraph (English)"
          value={form.overview}
        ...
        <textarea
          className="rounded border border-border px-3 py-2 text-xs"
          rows={4}
          placeholder="Overview paragraph (Odia)"
          value={form.overview_od}
          onChange={(e) => setForm((p) => ({ ...p, overview_od: e.target.value }))}
        />
