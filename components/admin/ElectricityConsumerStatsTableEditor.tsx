...
import { ELECTRICITY_CONSUMER_STATS_COLUMNS } from '../../lib/electricityConsumerStatsTable';

type RowState = ElectricityConsumerStatsRow & { clientId: string };

type Props = {
  departmentId: number;
  initialRows: ElectricityConsumerStatsRow[];
  onSave: (payload: { electricity_consumer_stats_rows: ElectricityConsumerStatsRow[] }) => Promise<void>;
};

function toRowState(rows: ElectricityConsumerStatsRow[]): RowState[] {
...
function toPayload(rows: RowState[]): ElectricityConsumerStatsRow[] {
  return rows
    .map((row, index) => {
      const out: ElectricityConsumerStatsRow = {};
    ...
    .filter((row) =>
      ELECTRICITY_CONSUMER_STATS_COLUMNS.some((col) => String(row[col.key] ?? '').trim() !== ''),
    );
}

export function ElectricityConsumerStatsTableEditor({ departmentId, initialRows, onSave }: Props) {
  ...
  const updateCell = (clientId: string, key: keyof ElectricityConsumerStatsRow, value: string) => {
    setRows((prev) =>
  ...
  const submit = async (e: FormEvent) => {
    ...
    setSaving(true);
    try {
      await onSave({ electricity_consumer_stats_rows: toPayload(rows) });
      setSaved(true);
      window.setTimeout(() => setSaved(false), 2000);
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : 'Could not save electricity consumer stats table.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <section className="rounded-lg border border-border bg-background p-4 text-sm">
      <h2 className="font-semibold text-text">Electricity consumer summary table</h2>
      <p className="mt-1 text-xs text-text-muted">
        Shown on the public Electricity department summary page (above office listing). Add rows for
        substation and consumer totals.
      </p>
