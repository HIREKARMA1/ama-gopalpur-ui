...
export function WaterPortfolioDashboard({
  ...
  const [detailTab, setDetailTab] = useState<'profile' | 'assets'>('profile');
          ...
          <div className="relative z-10">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-4">
              ...
              <div className="flex flex-wrap items-center justify-center sm:justify-start rounded-full bg-slate-100 p-1 w-full sm:w-auto">
                ...
                >
                  <Building size={14} />
                  <span>{language === 'or' ? 'ପ୍ରୋଫାଇଲ୍' : 'Profile'}</span>
                </button>
                <button
                  type="button"
                  onClick={() => setDetailTab('assets')}
                  className={`flex-1 sm:flex-none flex items-center justify-center gap-2 rounded-full px-3 py-1.5 text-xs font-semibold transition ${detailTab === 'assets'
                    ? 'bg-white text-[#0f172a] shadow-sm'
                    : 'text-[#64748b] hover:text-[#0f172a]'
                    }`}
                >
                  <Droplets size={14} />
                  <span>{language === 'or' ? 'ପାଣି ସମ୍ପତ୍ତି' : 'Water assets'}</span>
                </button>
              </div>
            </div>
            ...
            {detailTab === 'assets' && (
              <>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-6">
                  ...
                  {
                    label: 'Operational Capacity (MLD)',
                    val: waterProfile['operational_capacity_mld'],
                    icon: Activity,
                    color: 'violet',
                  },
                  {
                    label: 'Supply Hours / Day',
                    val: hours,
                    icon: Timer,
                    color: 'amber',
                  },
                ...
                {extraAttributes.length > 0 && (
                  <div className="border-t border-slate-200 pt-4 mt-2">
                    <h3 className="text-xs font-semibold uppercase tracking-wider text-slate-500 mb-3">
                      {language === 'or' ? 'ଅନ୍ୟାନ୍ୟ ବିବରଣୀ' : 'Additional attributes from CSV'}
                    </h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
