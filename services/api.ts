...
export const organizationsApi = {
  ...
  /** PS portfolio website image; stored under Departments/Education/{Org}/ps-portfolio/{section}/ */
  uploadPsPortfolioAsset: (id: number, file: File, assetType: string) => {
    const form = new FormData();
    form.append('file', file);
    form.append('asset_type', assetType);
    return apiFetch<{ url: string }>(`/api/v1/organizations/${id}/ps-portfolio-asset`, {
      method: 'POST',
      body: form,
    });
  },
  ...
  uploadAgriculturePortfolioAsset: (id: number, file: File, assetType: string) => {
    ...
    form.append('file', file);
    form.append('asset_type', assetType);
    return apiFetch<{ url: string }>(`/api/v1/organizations/${id}/agriculture-portfolio-asset`, {
      method: 'POST',
      body: form,
    });
  },
  /** Minor Irrigation portfolio image; stored under Departments/MinorIrrigation/{Org}/minor-portfolio/{section}/ */
  uploadMinorIrrigationPortfolioAsset: (id: number, file: File, assetType: string) => {
    const form = new FormData();
    form.append('file', file);
    form.append('asset_type', assetType);
    return apiFetch<{ url: string }>(`/api/v1/organizations/${id}/minor-irrigation-portfolio-asset`, {
      method: 'POST',
      body: form,
    });
  },
...
export const educationApi = {
  ...
  bulkDegreeCollegesCsv: (file: File) => {
    const form = new FormData();
    form.append('file', file);
    return apiFetch<{ imported: number; errors: string[] }>('/api/v1/education/bulk-csv/degree-colleges', {
      method: 'POST',
      body: form,
    });
  },
