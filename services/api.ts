...
// ----- Minor Irrigation (MIPs under Minor Irrigation department) -----
const minorIrrigationBase = (orgId: number) =>
  `/api/v1/minor-irrigation/organizations/${orgId}`;

export const minorIrrigationApi = {
  getProfile: (orgId: number) =>
    apiFetch<Record<string, unknown>>(
      `${minorIrrigationBase(orgId)}/profile`,
    ).catch(() => ({})),
  putProfile: (orgId: number, data: Record<string, unknown>) =>
    apiFetch<Record<string, unknown>>(
      `${minorIrrigationBase(orgId)}/profile`,
